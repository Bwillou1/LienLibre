/**
 * Pure JavaScript QR Code Generator for LienLibre
 * Zero external dependencies, client-side, 100% private.
...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    root.LienLibreQR = factory();
  }
}(typeof self !== 'undefined' ? self : this, function () {

  // QR Code Model & Matrix Builder
  ...
  QRCodeModel.prototype = {
    addData: function (data) {
      ...
      byteData.parsedData = stringToBytes(data);
  ...
  /**
   * Main High-Level Helper: Render QR Code onto an HTML5 Canvas
   * @param {string} text - URL or text to encode
   * @param {HTMLCanvasElement} canvas - target canvas element
   * @param {Object} options - styling options (size, colorDark, colorLight, margin)
   */
  function renderQRCode(text, canvas, options) {
    ...
    var ecl = options.errorCorrectionLevel || 0; // 0 = M (15% redundancy)

    var qr = new QRCodeModel(0, ecl);
    qr.addData(text);
    qr.make();

    var moduleCount = qr.getModuleCount();
    var totalCells = moduleCount + margin * 2;
    var cellSize = Math.floor(size / totalCells);
    var actualSize = cellSize * totalCells;
